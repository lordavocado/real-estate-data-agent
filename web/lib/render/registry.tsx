...
export const { registry } = defineRegistry(resightsCatalog, {
  components: {
    Card: ({ props, children }) => (
      <div
        className="w-full rounded-lg bg-card shadow-border"
        style={
          props.maxWidth ? { maxWidth: props.maxWidth } : undefined
        }
      >
        {props.title ? (
          <div className="px-4 py-3 text-sm font-medium tracking-tight text-foreground shadow-header-bottom">
            {props.title}
          </div>
        ) : null}
    ...
    Grid: ({ props, children }) => (
      <div
        className={cn("grid w-full", GAP[props.gap ?? "md"] ?? "gap-4")}
        style={{
    ...
    Metric: ({ props }) => (
      <div className="rounded-lg bg-card px-4 py-3 shadow-border">
        ...
        <div className="mt-1 text-xl font-semibold tracking-tight tabular-nums text-foreground">
          {props.value}
        </div>
        {props.detail ? (
    ...
    Progress: ({ props }) => {
      ...
      return (
        <div className="w-full">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-muted shadow-border">
            <div
    ...
    MapView: ({ props }) => (
      <div
        className="w-full min-w-0 overflow-hidden rounded-md shadow-border"
        style={{ height: props.height ?? CHART_HEIGHT }}
      >
        <ArtifactMap
          data={{
