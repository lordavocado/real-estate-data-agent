...
export default defineTool({
  description:
    "Generate a custom inline dashboard from a json-render spec (flat elements tree with root + elements). The web chat renders the spec live via React — charts (Recharts), maps (Leaflet), tables, metrics, and layout components. Also saves a self-contained HTML fallback to output/. Use when you need a bespoke dashboard, property overview, or investment summary that combines multiple visual elements. You generate the spec JSON — the tool validates and returns it for inline rendering.\n\n**Supported components:** Card, Stack (flex layout), Grid (CSS grid), Heading (h1-h4), Text (paragraph), Badge, Separator, Metric (KPI stat), Table (data table), BarChart, LineChart, PieChart, Progress, List (icon+text items), MapView (Leaflet markers).\n\n**Data binding:** Use `{ \"$state\": \"/path/to/value\" }` in props to reference values from the `data` object. Pass all dynamic values in the `data` parameter.",
  ...
  async execute({ title, root, elements, data = {}, theme = "blue", height = 600 }) {
    ...
    const summary = [
      `## ${title}`,
      ``,
      `Custom dashboard med **${elementCount} elementer** — vist inline i chatten.`,
      ``,
      `HTML-fallback gemt som \`${filename}\` (\`${filePath}\`).`,
    ].join("\n");
